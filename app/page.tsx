  ...
  return (
    <div className='min-h-[calc(100vh-64px)] flex flex-col justify-start items-center px-8 py-4 gap-4'>
      ...
      <div className='grid grid-cols-3 max-sm:grid-cols-1 gap-4'>
        {data
        .filter((item) => action==='Active' && item.isActive===true || action==='All' || action==='Inactive' && item.isActive===false ||!action) 
        .filter((item) => action==='Inactive' && item.isActive===false || action==='All' || action==='Active' && item.isActive===true ||!action)
        .map((item) => 
        <Card key={item.id}>
        ...
        <CardFooter className='flex flex-row items-center justify-between gap-4'>
          <Button variant={'outline'} className='border border-primary rounded-2xl'>Remove</Button>
